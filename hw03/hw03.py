...
def interleaved_sum(num, f_odd, f_even):
    ...
    """
    "*** YOUR CODE HERE ***"
    def sum_from_even(now):
        if now > num: return 0
        return f_even(now) + sum_from_odd(now+1)
     
    def sum_from_odd(now):
        if now > num: return 0
        return f_odd(now) + sum_from_even(now+1)
...
def count_dollars(sum_needed):
    ...
    """
    "*** YOUR CODE HERE ***"
    def f(amount, bill_max):
        if amount == 0: return 1
        if amount <0 : return 0
        if bill_max is None: return 0
        return f(amount - bill_max, bill_max) + f(amount, next_smaller_dollar(bill_max))
    return f(sum_needed, 100)
...
def count_dollars_upward(sum_needed):
    ...
    def f(amount, bill_max):
        ...
        if amount <0 : return 0
        if bill_max is None: return 0
        return f(amount - bill_max, bill_max) + f(amount, next_larger_dollar(bill_max))
